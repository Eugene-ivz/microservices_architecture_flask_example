...
class TokenBlocklist(db.Model):
    ...
    jti: Mapped[uuid.UUID] = mapped_column(default=uuid.uuid4, index=True, nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(default=datetime.datetime.now(datetime.UTC), nullable=False)
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("app_users.id"),default=lambda: get_current_user().id , nullable=False)
