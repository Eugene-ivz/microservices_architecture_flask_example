...
def test_create_delete_user(session):
    ...
    assert user.password == session.scalar(select(User).where(User.username == user.username)).password
    session.delete(user)
    session.commit()
...
def test_existing_user(session):
    ...
    assert user.username == session.scalar(select(User).where(User.username == user.username)).username
