...
def create_user(request):
    ...
    response = httpx.post(
        f'http://{os.getenv("FLASK_AUTH_USER_REGISTRATION")}', data=data
    )
...
def get_jwt(request):
    ...
    response = httpx.post(f'http://{os.getenv("FLASK_AUTH_CREATE_JWT")}', auth=basic)
    if response.status_code == 200:
...
def validate_jwt(request):
    ...
    response = httpx.post(
        f'http://{os.getenv("FLASK_AUTH_VALIDATE_JWT")}',
...
def logout_user(request):
    headers = {"X-CSRF-TOKEN": request.cookies.get("csrf_access_token")}
    response = httpx.post(
        f'http://{os.getenv("FLASK_AUTH_LOGOUT")}',
        cookies=request.cookies,
        headers=headers,
    )
    if response.status_code == 200:
